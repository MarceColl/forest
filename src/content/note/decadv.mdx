...
```lisp
(defun some-ruleset ()
  (let ((running t) (start 1) (a 0) (b 0) (c 0) (d 0))
    (loop while running
      do (cond
           ((and (> start 0))
             (decf start)
...
and the real common lisp code has some extra declarations and optimizations using FIXNUMs so the compiler uses
registers and native operations to perform arithmetic.
